...
export default class CommandHandler {
	...
	private async setUp(
		...
		for ( const [file, fileName] of getAllFiles(path.join(__dirname, "commands")) ) {
			if ( disabledDefaultCommands.includes(fileName) ) continue;
	...
	public async registerCommand(
		...
		if ( run || execute ) throw new Error(`Command located at "${file}" has either a "run" or "execute" function. Please rename that function to "callback".`);
		let names = commands || aliases || [];
		if ( !name && ( !names || names.length === 0 ) ) throw new Error(`Command located at "${file}" does not have a name, commands array, or aliases array set. Please set at lease one property to specify the command name.`);
		if ( typeof names === "string" ) names = [names];
		if ( name && !names.includes(name.toLowerCase()) ) names.unshift(name.toLowerCase());
		...
		if ( missing.length && instance.showWarns ) console.warn(`WOKCommands > Command "${names[0]}" does not have the following properties: ${missing}.`);
		if ( testOnly && !instance.testServers.length ) console.warn(`WOKCommands > Command "${names[0]}" has "testOnly" set to true, but no test servers are defined.`);
		if ( slash !== undefined && typeof slash !== "boolean" && slash !== "both" ) throw new Error(`WOKCommands > Command "${names[0]}" has a "slash" property that is not boolean "true" or string "both".`);
		if ( !slash && options.length ) throw new Error(`WOKCommands > Command "${names[0]}" has an "options" property but is not a slash command.`);
		if ( slash && !( builtIn && !instance.isDBConnected() ) ) {
			if ( !description ) throw new Error(`WOKCommands > A description is required for command "${names[0]}" because it is a slash command.`);
			if ( minArgs !== undefined && !expectedArgs ) throw new Error(`WOKCommands > Command "${names[0]}" has "minArgs" property defined without "expectedArgs" property as a slash command.`);
			if ( options.length ) {
	...
	public getCommandsByCategory(category: string, visibleOnly?: boolean): ICommand[] {
		const results: ICommand[] = [];
		for ( const command of this.commands ) {
			if ( visibleOnly && command.hidden ) continue;
			if ( command.category === category ) results.push(command);
		}
		return results;
	}
