import { ApplicationCommand, ApplicationCommandOptionData, Client, CommandInteraction, CommandInteractionOptionResolver, GuildMember, MessageEmbed } from "discord.js";
...
class SlashCommands {
	...
	private async setUp(listen: boolean, typeScript = false) {
		...
		for ( const [file, fileName] of getAllFiles(path.join(__dirname, "command-checks")) ) this._commandChecks.set(fileName, require(file));
	...
	public async invokeCommand(interaction: CommandInteraction, commandName: string, options: CommandInteractionOptionResolver, args: string[]) {
		const command = this._instance.commandHandler.getCommand(commandName);
